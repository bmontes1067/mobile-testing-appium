"""
Downloads Sauce Labs demo apps.
Usage:
...
def get_latest_asset_url(repo: str, asset_suffix: str, exclude: str = None) -> str:
    ...
    raise RuntimeError(f"No asset with suffix '{asset_suffix}' found in the latest release of {repo}")
...
def download(platform: str):
    ...
    if os.path.exists(dest):
        print(f"[{platform}] Already exists: {dest} — skipping.")
        return
    os.makedirs(APPS_DIR, exist_ok=True)
    print(f"[{platform}] Querying GitHub API...")
    try:
        url = get_latest_asset_url(info["repo"], info["asset_suffix"], info.get("exclude"))
    except Exception as e:
        print(f"[{platform}] ❌ {e}")
        sys.exit(1)
    print(f"[{platform}] Downloading {url} ...")
